...
public class ReportImpl extends RequestImpl implements Report {
    ...
    private static Logger logger = LoggerFactory.getLogger(ReportImpl.class);

    public ReportImpl(FromPathHeaderImpl from, ToPathHeaderImpl to, StatusHeaderImpl status,
            MessageIDHeaderImpl messageID) {
        setHeader(from);
        setHeader(to);
        setHeader(messageID);
        this.status = status;
    ...
    public void parse(RawMsrpPacket rp) {
        for (RawMsrpHeader<?> h : rp.getHeaders()) {
            try {
                ...
                MsrpHeaderImpl mh = MsrpParser.headerTypes.get(h.getKey()).newInstance();
                // mh.parse(h.getKey() + h.getContent());
                setHeader(mh);
            } catch (InstantiationException e) {
                e.printStackTrace();
                logger.error("The header " + h.getKey() + " has no nullary constructor defined!");
